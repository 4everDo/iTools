...
+ (THUD *)instance {
    ...
    static THUD *hudInstance = nil;
    dispatch_once(&once, ^{
        hudInstance = [[THUD alloc] init];
    });
    return hudInstance;
}

+ (void)showContent:(NSString *)content {
    [THUD showImage:nil content:content withTime:kDefaultTime needLock:NO inView:nil];
}

+ (void)showContent:(NSString *)content needLock:(BOOL)lock {
    [THUD showImage:nil content:content withTime:kDefaultTime needLock:lock inView:nil];
}

+ (void)showContent:(NSString *)content withTime:(CGFloat)time {
    [THUD showImage:nil content:content withTime:time needLock:NO inView:nil];
}

+ (void)showContent:(NSString *)content withTime:(CGFloat)time inView:(UIView *)view {
    [THUD showImage:nil content:content withTime:time needLock:NO inView:view];
}

+ (void)showImage:(UIImage *)image {
    [THUD showImage:image content:nil withTime:kDefaultTime needLock:NO inView:nil];
}

+ (void)showImage:(UIImage *)image content:(NSString *)content withTime:(CGFloat)time {
    [THUD showImage:image content:content withTime:time needLock:NO inView:nil];
}

+ (void)showImage:(UIImage *)image content:(NSString *)content withTime:(CGFloat)time inView:(UIView *)view{
    [THUD showImage:image content:content withTime:time needLock:NO inView:view];
}

+ (void)showImage:(UIImage *)image content:(NSString *)content needLock:(BOOL)lock inView:(UIView *)view {
    [THUD showImage:image content:content withTime:0 needLock:lock inView:view];
}

+ (void)showImage:(UIImage *)image content:(NSString *)content withTime:(CGFloat)time needLock:(BOOL)lock inView:(UIView *)view {
    [[THUD instance] showImage:image content:content activity:NO withTime:time needLock:lock inView:view];
}

+ (void)showActivityIndicatorInView:(UIView *)view needLock:(BOOL)lock {
    [[THUD instance] showImage:nil content:nil activity:YES withTime:0 needLock:lock inView:nil];
}

+ (void)dismiss {
    [[THUD instance] hudHide];
}
